# 特訓カレンダー（tokkun-calendar）

学習塾の校舎で、**講師の空き時間を一目で判断する**ための社内ツール。
Salesforce に入っている授業（特訓）データを読み取り、「講師 × 時間」のマトリクスで週単位に表示する。
...
- 特訓ありは青、体験は緑、途中で終わる枠はオレンジ（マウスオーバーで「8/17〜空」）、空きは空白
...
- 表の下のプルダウンで、まだ特訓が確定していない在籍講師をその曜日に追加

どちらも `(講師, 曜日, 時刻)` の週単位で全員に共有される。
**描画時は常に Salesforce を優先する**ので、特訓が入れば手で付けた印は隠れ、
足した講師は通常の段へ移る。保存値は消さないため、特訓がなくなれば元の印が戻る。
